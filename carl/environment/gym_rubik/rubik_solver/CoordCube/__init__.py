            ...
            if CoordCube.getPruning(slice_urf_to_dlf_parity_prun, i) == depth:
                for j in [0, 1, 2, 4, 7, 9, 10, 11, 13, 16]:
                    ...
                    if (CoordCube.getPruning(
                            slice_urf_to_dlf_parity_prun,
                        ((CoordCube.N_SLICE2 * newURFtoDLF) + newSlice) * 2 + newParity,
                    ) == 0x0F):
                        CoordCube.setPruning(
                            slice_urf_to_dlf_parity_prun,
                            ((CoordCube.N_SLICE2 * newURFtoDLF) + newSlice) * 2 + newParity,
                            depth + 1,
                        )
        ...
        for i in range(CoordCube.N_SLICE2 * CoordCube.N_URtoDF * CoordCube.N_PARITY):
            ...
            if depth == CoordCube.getPruning(slice_ur_to_df_parity_prun, i):
                for j in [0, 1, 2, 4, 7, 9, 10, 11, 13, 16]:
                    ...
                    newParity = CoordCube.parityMove[parity][j]
                    if (CoordCube.getPruning(
                            slice_ur_to_df_parity_prun,
                        (CoordCube.N_SLICE2 * newURtoDF + newSlice) * 2 + newParity,
                    ) == 0x0F):
                        CoordCube.setPruning(
                            slice_ur_to_df_parity_prun,
                            (CoordCube.N_SLICE2 * newURtoDF + newSlice) * 2 + newParity,
...
CoordCube.MergeURtoULandUBtoDF = read_or_func_matrix('merge_ur_to_ul_and_ub_to_df_move.csv',
                                                     build_merge_ur_to_ul_and_ub_to_df)

CoordCube.Slice_URFtoDLF_Parity_Prun = read_or_func_list('slice_urf_to_dlf_parity_prun.csv',
                                                         build_slice_urf_to_dlf_parity_prun)

CoordCube.Slice_URtoDF_Parity_Prun = read_or_func_list('slice_ur_to_df_parity_prun.csv',
                                                       build_slice_ur_to_df_parity_prun)
