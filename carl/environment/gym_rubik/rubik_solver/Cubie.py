...
class Sticker:
    ...
    def __init__(self, color):
        if color.lower() not in self.COLOURS:
            raise ValueError('Color {} is not one of {}'.format(color, ', '.join(self.COLOURS)))
    ...
    def __init__(self, **kwargs):
        ...
        for key, value in kwargs.items():
            ...
            if key not in self.FACINGS:
                raise ValueError('Face {} is not one of {}'.format(key, ', '.join(list(self.FACINGS))))

            self.facings[key] = Sticker(value)

    def __repr__(self):
        return '<{}: {}>'.format(self.__class__.__name__, ', '.join([f'{k}: {v}' for k, v in self.facings.items()]))
...
class Cube:
    ...
    def __reset_cube(self):
        ...
        for cubie in self.CUBIES:
            ...
            if len(cubie) == 3:
                self.cubies[cubie] = Corner(**{face: Cubie.facing_to_color(face) for face in cubie})
            elif len(cubie) == 2:
                self.cubies[cubie] = Edge(**{face: Cubie.facing_to_color(face) for face in cubie})
            else:
                self.cubies[cubie] = Center(**{face: Cubie.facing_to_color(face) for face in cubie})
