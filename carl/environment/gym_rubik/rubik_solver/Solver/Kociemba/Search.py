...
class Search:
        ...
        while True:
                ...
                if (depthPhase1 - n) > Search.minDistPhase1[n + 1] and not busy:
                    ...
                    if Search.po[n] > 3:
                        while True:
                            ...
                            if Search.ax[n] > 5:
                                ...
                                if n == 0:
                                    if depthPhase1 >= maxDepth:
                                        raise NoSolution('No solution exists for the given maxDepth')
                                    else:
                                        depthPhase1 += 1
                                        Search.ax[n] = 0
                                        Search.po[n] = 1
                            ...
                            if not (n != 0 and
                                    (Search.ax[n - 1] == Search.ax[n] or Search.ax[n - 1] - 3 == Search.ax[n])):
                                break
                    else:
                        busy = False
                if not busy:
            ...
            Search.minDistPhase1[n + 1] = max(
                CoordCube.CoordCube.getPruning(
                    ...
                    CoordCube.CoordCube.N_SLICE1 * Search.flip[n + 1] + Search.slice[n + 1],
                ),
                CoordCube.CoordCube.getPruning(
                    CoordCube.CoordCube.Slice_Twist_Prun,
                    CoordCube.CoordCube.N_SLICE1 * Search.twist[n + 1] + Search.slice[n + 1],
                ),
            )
            # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ...
            if Search.minDistPhase1[n + 1] == 0 and n >= (depthPhase1 - 5):
                ...
                if n == (depthPhase1 - 1):
                    ...
                    if s >= 0:
                        if s == depthPhase1 or (Search.ax[depthPhase1 - 1] != Search.ax[depthPhase1]
    ...
    def totalDepth(depthPhase1, maxDepth):
        ...
        d1 = CoordCube.CoordCube.getPruning(
            ...
            (CoordCube.CoordCube.N_SLICE2 * Search.URFtoDLF[depthPhase1] + Search.FRtoBR[depthPhase1]) * 2 +
        ...
        d2 = CoordCube.CoordCube.getPruning(
            ...
            (CoordCube.CoordCube.N_SLICE2 * Search.URtoDF[depthPhase1] + Search.FRtoBR[depthPhase1]) * 2 +
            Search.parity[depthPhase1],
        )
        ...
        while True:
            ...
            Search.URFtoDLF[n + 1] = CoordCube.CoordCube.URFtoDLF_Move[Search.URFtoDLF[n]][mv]
            ...
            Search.minDistPhase2[n + 1] = max(
                CoordCube.CoordCube.getPruning(
                    CoordCube.CoordCube.Slice_URtoDF_Parity_Prun,
                    (CoordCube.CoordCube.N_SLICE2 * Search.URtoDF[n + 1] + Search.FRtoBR[n + 1]) * 2 +
                    Search.parity[n + 1],
                ),
                CoordCube.CoordCube.getPruning(
                    CoordCube.CoordCube.Slice_URFtoDLF_Parity_Prun,
                    (CoordCube.CoordCube.N_SLICE2 * Search.URFtoDLF[n + 1] + Search.FRtoBR[n + 1]) * 2 +
                    Search.parity[n + 1],
                ),
            )
            # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
