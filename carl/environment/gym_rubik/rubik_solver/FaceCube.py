...
class FaceCube:
    ...
    def toCubieCube(self):
        ...
        for i in Corner.reverse_mapping.keys():
            ...
            for ori in range(3):
                if (self.f[FaceCube.cornerFacelet[i][ori]] == Color.U) or (self.f[FaceCube.cornerFacelet[i][ori]]
                                                                           == Color.D):
                    break
            else:
                ori = 3
        ...
        for i in Edge.reverse_mapping.keys():
            for j in Edge.reverse_mapping.keys():
                if (self.f[FaceCube.edgeFacelet[i][0]]
                        == FaceCube.edgeColor[j][0]) and (self.f[FaceCube.edgeFacelet[i][1]]
                                                          == FaceCube.edgeColor[j][1]):
                    ccRet.ep[i] = j
                    ccRet.eo[i] = 0

                if (self.f[FaceCube.edgeFacelet[i][0]]
                        == FaceCube.edgeColor[j][1]) and (self.f[FaceCube.edgeFacelet[i][1]]
                                                          == FaceCube.edgeColor[j][0]):
                    ccRet.ep[i] = j
                    ccRet.eo[i] = 1
                    break
        return ccRet
