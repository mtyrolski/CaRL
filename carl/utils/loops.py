...
from typing import Generator, Iterator, Mapping, cast
    ...
    def iterate_training_modules(self) -> Iterator[tuple[str, TrainingModule]]:
        for name, component in self.components.items():
            ...
            if training_module is None:
                continue
            if isinstance(training_module, dict):
...
MetricValue = str | float | int | list[float]


@dataclass
class Logs:
    prefix: str = ""
    content: dict[str, MetricValue] = field(default_factory=dict)

    def update(self, metrics: Mapping[str, MetricValue]) -> 'Logs':
...
def get_next_batch(online_loader_generator: Iterator, loop_control: LoopControl) -> list[ProblemInstance]:
    raw_batch = next(online_loader_generator)
    batch_tensor = cast(torch.Tensor, raw_batch)
...
def solve_trajectories(solver: Solver, online_boards: list[ndarray], n_jobs: int) -> tuple[list[Experience], Logs]:
    outputs_all_at_once = list(
        joblib.Parallel(n_jobs=n_jobs, verbose=100)(
...
def train_components(components: ComponentCollection, replay_buffer: SimpleUniversalReplayBuffer,
    ...
    for component_name, component in components.components.items():
        ...
        if isinstance(network_container, dict):
            ...
            assert isinstance(network_container, PreTrainedModel) and isinstance(buffer_container,
                                                                                  OfflineReplayBuffer)
            logger.info(f"Training component {component_name}")
...
def evaluate_components(solver: Solver, eval_instance_generator: InstanceGenerator | None) -> Logs:
    ...
    for batch_of_instances in eval_instance_generator.reset_dataloader():
        ...
        batch = list(batch_tensor.cpu().numpy())
        outputs = list(
            joblib.Parallel(n_jobs=1, verbose=100)(
                joblib.delayed(solver.solve)(initial_state) for initial_state in batch
            )
        )
        outputs = cast(list[Experience], outputs)
